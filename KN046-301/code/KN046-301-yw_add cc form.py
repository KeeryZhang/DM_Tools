...
def data1(ws1, keys1):
    ...
    for row in range(7, ws1.max_row+1):
        ...
        real_visit = ws1[keys1['实际访视时间']+str(row)].value
        drug = ws1[keys1['当前使用药物']+str(row)].value
        skip = ws1[keys1['是否跳过访视']+str(row)].value

        yy, mm, dd = real_visit.split("-")
        real_visit = datetime(int(yy), int(mm), int(dd))

        data_ws.setdefault(subject, {})
        data_ws[subject].setdefault("rand_date", None)
        if "再次随机" in instance_raw:
            data_ws[subject]["rand_date"] = real_visit
            continue
        elif "D" in instance_raw:
        ...
        if (instance+"_1") not in data_ws[subject]:
            data_ws[subject].setdefault(instance+"_1", {'row': row, 'drug': drugset, 'skip': skip, 'real_visit': real_visit})
        else:
            data_ws[subject].setdefault(instance+"_2", {'row': row, 'drug': drugset, 'skip': skip, 'real_visit': real_visit})
    
    data_ws = refine_instance(data_ws)
    return data_ws

def refine_instance(data_ws_raw):
    data_ws = deepcopy(data_ws_raw)
    for subject in data_ws_raw:
        pid = data_ws_raw[subject]
        for instance in pid:
            if instance == "rand_date":
                continue
            ipid = pid[instance]
            if pid["rand_date"] is not None and ipid["real_visit"] >= pid["rand_date"]:
                instance_name = "C" + instance.split("_")[0]
            elif pid["rand_date"] is None or (pid["rand_date"] is not None and ipid["real_visit"] < pid["rand_date"]):
                instance_name = instance.split("_")[0]
            data_ws[subject].setdefault(instance_name, ipid)
            data_ws[subject].pop(instance)
            if "CC" not in instance_name and "nnd" in data_ws[subject][instance_name]["drug"]:
                data_ws[subject][instance_name]["drug"].remove("nnd")

        data_ws[subject].pop("rand_date")
...
def visitcheck(data_ws1, data_ws2, ws1):
    ...
    for id in data_ws1:
        ...
        for instance in pid_ws1:
            ...
            if not visitpass and not iderror and not instanceerror and not exynpass:
                if "CC" in instance and "nnd" in ipid_ws1["drug"]:
                    cccheck = True
                else:
                    cccheck = False
            ...
            if visitpass:
                ...
                if cccheck and "NA" in ipid_ws2['drug']:
                    rsg = "Info: O药无药物编号"
                elif ipid_ws1['drug'] == ipid_ws2['drug']:
                    rsg = 'Info: 该行使用药物匹配成功'
                else:
...
def gycheck(data_ws1, data_ws2, ws1):
    ...
    for id in data_ws1:
        ...
        for instance in pid_ws1:
            ...
            if not visitpass and not iderror and not instanceerror and not exynpass:
                if "CC" in instance and "NA" in ipid_ws1["drug"]:
                    cccheck = True
                else:
                    cccheck = False
            ...
            if visitpass:
                ...
                if cccheck and "nnd" in ipid_ws2['drug']:
                    rsg = "Info: O药无药物编号"
                elif ipid_ws1['drug'] == ipid_ws2['drug']:
                    rsg = 'Info: 该行使用药物匹配成功'
                else:
